...
export default class TableIxmiquilpan extends Component {
  constructor () {
    super()
    this.state = {
      nuevo: '',
  ...
  componentDidMount () {
    const itemsRef = firebaseConf.database().ref('agenda-cita/ixmiquilpan')
    this.listenForItems(itemsRef)
  }

  update = (item) => {
    let updates = {}
    updates['agenda-cita/ixmiquilpan/' + item.id] = {
      status: "Atendido",
      nombre: item.nombre,
